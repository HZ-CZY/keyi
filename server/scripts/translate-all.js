 ...
 * 
 * 用法: node scripts/translate-all.js
 */
...
const dbPath = path.join(__dirname, '..', 'data', 'keyi.db');
...
function getApiKey() {
  try {
    ...
    console.error('Could not read API key:', e.message);
  }
...
async function main() {
  ...
  // Read all notes from deck_id=5 (admin's 文言文 deck)
  ...
  for (let i = 0; i < notes.length; i += BATCH_SIZE) {
    ...
    if (toTranslate.length === 0) {
      ...
      const stmt = db.prepare('UPDATE notes SET notetype_id = ?, modified_at = unixepoch() WHERE id = ? AND notetype_id != ?');
      for (const n of batch) {
        stmt.run(9, n.id, 9);
      }
    ...
    for (const n of toTranslate) {
      const fields = JSON.parse(n.fields);
      sentences.push(stripHtml(fields[0]));
    }
    ...
    try {
      ...
      if (translations.length < toTranslate.length) {
        console.log('  Warning: got ' + translations.length + ' translations, expected ' + toTranslate.length);
      }
    } catch (err) {
