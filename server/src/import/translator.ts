/**
 * Shared translator utilities for ancient Chinese → modern Chinese
 * Used by both the import route and translate route
 */

const TRANSLATION_NOTETYPE_NAME = '古文翻译';

// Check if a deck name contains ancient Chinese keywords
export function isClassicalChineseDeck(deckName: string): boolean {
...
export function getOrCreateTranslationNotetype(db: any, userId: number): number {
...
export async function translateDeckNotes(
  db: any,
  deckId: number,
  userId: number
  ...
  if (!apiKey) {
    console.log('[Translate] DEEPSEEK_API_KEY not set, skipping translation');
    return { translated: 0, failed: 0 };
  }

  // Get notes without translation
  ...
  for (let i = 0; i < toTranslate.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`[Translate] API error: ${response.status}: ${errText.substring(0, 100)}`);
        failed += batch.length;
        continue;
      }
  ...
  if (translated > 0) {
    console.log(`[Translate] Deck #${deckId} "${deck.name}": ${translated} translated, ${failed} failed`);
  }
